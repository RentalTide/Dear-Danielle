/**
 * Transaction process graph for fashion rentals:
 *   - fashion-rental
 *
 * This process uses manual capture (preauthorization) pattern:
 *   1. Customer pays - payment is authorized (held, not captured)
 *   2. Provider accepts - payment is captured
 *   3. Auto-capture after 7 days if provider doesn't respond
 *   4. Auto-decline/refund if provider rejects or doesn't respond within 14 days
 */
...
export const transitions = {
  ...
  // Note: In fashion-rental, confirm-payment only authorizes (holds) the payment,
  // it does NOT capture it. The payment moves to preauthorized state.
  ...
  // Provider accepts the rental request, which captures the held payment
  ACCEPT: 'transition/accept',

  // If the provider doesn't respond within 7 days, the payment is automatically captured
  AUTO_CAPTURE: 'transition/auto-capture',

  // Provider declines the rental request, which refunds the held payment
  DECLINE: 'transition/decline',

  // If the provider doesn't respond within 14 days, the payment is automatically refunded
  AUTO_DECLINE: 'transition/auto-decline',

  // Provider or operator can mark the product shipped/delivered
...
export const graph = {
  ...
  states: {
    ...
    [states.PREAUTHORIZED]: {
      on: {
        [transitions.ACCEPT]: states.PURCHASED,
        [transitions.AUTO_CAPTURE]: states.PURCHASED,
        [transitions.DECLINE]: states.DECLINED,
        [transitions.AUTO_DECLINE]: states.DECLINED,
      },
    },
...
export const statesNeedingProviderAttention = [states.PREAUTHORIZED, states.PURCHASED];
